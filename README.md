...
| `CODEX_BIN` | 本地 ChatGPT/Codex 命令行工具的绝对路径（用于辅助调起后台服务） | `/Applications/ChatGPT.app/Contents/Resources/codex` |
...
| `/np` | 交互式选择本地项目，并在后续输入信息时在该项目下自动创建新会话绑定工作 | `/np` |
...
| `/cancel` 或 `/stop` 或 `/s` | 终止/中断当前会话正在运行的任务 | `/s` |
