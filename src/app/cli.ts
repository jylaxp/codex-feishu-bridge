...
function helpText(): string {
  return [
    ...
    'Configuration is loaded from ~/.codex-feishu-bridge/config.toml by default.',
    'Legacy config.json/.env files are one-time migration sources, then removed after config.toml is written.',
    'Process/service-manager environment values override config.toml at runtime.',
    'setup creates the private config.toml and scans a Feishu QR code when no bot is configured.',
    'run/start/restart also invoke setup automatically when credentials are missing.',
    'rebind forces a new Feishu QR-code app registration and updates lark-bots.json.',
