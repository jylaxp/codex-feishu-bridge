...
- R14. 多机器人扫码注册必须创建或替换恰好一个 bot record。一次 QR 扫码注册一组 Lark app 凭证，获取机器人身份/名称，生成内部 `botKey`，且不得覆盖其他机器人、认领 owner 或把任何群绑定到 ChatGPT thread。
...
- 引入稳定的 `botKey` 作为每个机器人的内部身份。`botKey` 不是密钥；它是 binding、card action、health、logs 和 idempotency scope 使用的路由 key。
- 多机器人凭证存放在 Bridge config home 下的 `lark-bots.json`。旧 `.env` 里的 `LARK_APP_ID` 和 `LARK_APP_SECRET` 保留为 default bot 的兼容来源。
- 保留 `botKey = "default"` 作为 legacy 单机器人兼容 key。`bot add` 永远不能生成这个 key。
...
- 绑定身份升级为 `botKey + tenantKey + chatId`。一个 binding 指向一个 ChatGPT thread，并存储该 bot/chat pair 的静态执行设置。
...
- 群内目标管理是一等交互。当 owner 在群里发送 `@current bot /bind`、`/unbind`、`/model`、`/cwd` 或 access 命令时，目标就是当前 `botKey + tenantKey + chatId`。
...
- 拆分三个生命周期概念：QR 凭证注册（`botKey -> appId/appSecret/bot identity`）、owner 认领（`botKey -> tenant/owner/admins`）和群 thread 绑定（`botKey + tenantKey + chatId -> threadId`）。任何命令都不应静默执行多个生命周期步骤。
- 拆分移除语义：
...
- 使用读兼容 legacy migration。如果 `lark-bots.json` 不存在且 legacy `.env` 包含有效 `LARK_APP_ID`/`LARK_APP_SECRET`，Bridge 在 runtime 合成保留的 `default` bot。现有 `bindings.json` schema v1 记录按 `default + tenantKey + chatId` bindings 读取。
...
- 现有 `setup` 命令保持向后兼容：当 legacy `LARK_APP_ID` 和 `LARK_APP_SECRET` 缺失时，它注册 default bot。
- 额外机器人使用显式 bot 子命令。推荐 CLI 形态：
  - `codex-feishu-bridge bot add` 打印一个 QR code，注册一个 Lark app，获取机器人 identity/name，生成内部 `botKey`，并写入一个 bot entry。
  - `codex-feishu-bridge bot import --app-id cli_xxx --app-secret ...` 不扫码，导入已有 Lark app，然后获取 identity/name 并生成内部 `botKey`。
  - `codex-feishu-bridge bot migrate-default` 读取现有 legacy credentials，探测当前机器人 identity/name，并在不扫码的情况下物化保留的 default bot。
...
- `botKey` 不属于正常 add 流程。它由 Bridge 生成、持久化到 `lark-bots.json`，只在诊断或高级输出中展示。
- 机器人 display name 在凭证可用后，从已注册 app/bot identity 获取。用户提供的 display name 只作为可选 alias，不是事实来源。
- QR 注册只写 credentials。机器人保持 `unclaimed`，直到 owner 在私聊中认领它。
...
- 对已有 bot record 执行 rebind 时，`botKey` 保持稳定。它是 Bridge routing key；`appId` 是该 key 背后的 Lark app identity。

### BotKey 生成规则

- 正常 `bot add` 或 `bot import` 期间，operator 永远不提供 `botKey`。
- 对新 bot record，从标准化的 Lark app identity 生成候选 key，而不是从机器人名称生成：
  - 标准 seed：`lark-app:${appId.toLowerCase()}`。
  - Digest：`sha256(seed)`。
  - 候选 key：`bot_${base32url(digest).toLowerCase().slice(0, 12)}`。
- 生成的 key 对同一个 `appId` 稳定，不暴露原始 `appId`，且不受机器人 display name 变化影响。
- `default` 保留给 legacy 单 bot 兼容，永远不能被生成。
- 如果生成 key 与另一个不同 bot 冲突，则每次多取 4 个 digest 字符扩展后缀，直到唯一；如果在最大 key 长度内仍无法唯一，则 fail closed。
- 如果同一个 `appId` 已经存在于 `lark-bots.json`，`bot add` 或 `bot import` 不得创建重复记录；应引导 operator 去 list/rebind/enable 现有 bot。
- 对 `bot rebind`，不重新生成 `botKey`。选中的 existing bot record 保留当前 `botKey`，即使新的 QR registration 返回了不同 `appId`；随后 owner/admin 和 group state 需要重新验证。
- `displayName` 单独获取和持久化。它只用于人类可读的 cards 和 selectors，永远不作为 routing key。

### Legacy Upgrade Compatibility

- 升级时，只有 legacy `.env` bot credentials 的现有安装被视为一个 enabled bot，保留 `botKey = "default"`。
- 启动不得强制 QR 重新注册。如果 legacy credentials 有效，Bridge 可以直接从 `.env` 启动 default bot runtime。
- 启动不得强制聊天重新绑定。现有 `bindings.json` schema v1 记录通过在内存中注入 `botKey = "default"` 加载为 default-bot bindings。
- Legacy `.env` policy fields 映射到 default bot：
  - `LARK_TENANT_KEY` 成为 default bot tenant scope。
  - `ALLOWED_CHATS` 成为 default bot allowed/discovered chat scope。
  - `AUTHORIZED_USERS` 为兼容现有 binding permissions，成为初始 default bot owner/admin management set。
  - `ALLOWED_APPROVERS` 仍是 approval decision set；如果为空，仍使用现有 owner/admin bootstrap rules。
- 如果无法从 legacy config 推断 owner/admin，default bot 对 management actions 进入 `claim_required`，但已经绑定且允许的 chats 仍保留其 thread bindings。owner 必须先 claim 才能改设置。
- 持久迁移采用 write-through。只读的 `run`、`status` 或 `doctor` 可以使用 legacy files 运行，不重写文件。`bot add`、显式 migration，或下一次 binding/config mutation 会物化 `lark-bots.json`，并用升级后的 schema 原子写入 `bindings.json`。
- 物化之前仍可 rollback，因为只读启动不会破坏性改写 legacy `.env` 和 v1 bindings。

### Legacy 默认机器人迁移指令

- 增加 `codex-feishu-bridge bot migrate-default`，作为已经通过 legacy `.env` keys 配置过一个机器人的安装实例的正式迁移路径。
- 该命令不接受 operator 传入 `botKey`、机器人名称或 open ID。它读取 legacy `LARK_APP_ID`、`LARK_APP_SECRET` 和 domain settings，然后使用 app authentication 解析机器人 identity。
- Identity probe：
...
- 目标 `botKey` 永远是保留的 `default` key。该命令迁移现有 single-bot installation；不能 hash 生成新的 bot key，否则会强制现有 v1 bindings 改名。
- 默认模式是 preview：打印已脱敏的 migration plan，包括目标 `botKey`、解析到的 `displayName`、脱敏后的 `botOpenId`、activation status、待升级 bindings 数量，以及 owner/admin policy source。`--write` 原子物化该 plan；非交互 automation 也可以传 `--yes`。
...
- 该命令是幂等的。如果 `lark-bots.json` 已经包含 app ID 或 bot open ID 相同的 `default`，重复执行只刷新 display metadata，并保持 bindings 不变。如果 `default` 已存在但 app ID/open ID 不同，命令停止，并要求显式走 `bot rebind` 或人工 conflict resolution path。
...
- 群列表按 `botKey + tenantKey + chatId` keyed，而不是按群名。卡片可以显示群名便于阅读，但必须同时包含短 chat ID 后缀和 discovery source，以避免同名群歧义。
...
- 群内 owner 管理卡片和 action tokens 包含 `botKey`、`tenantKey`、`chatId`、operator open ID、binding revision 和 TTL。
...
- 首选发现来源：订阅该机器人的 bot-added-to-group event，携带 `botKey`、`tenantKey` 和 `chatId`。
...
**测试场景：**
- Happy path：现有单 bot `.env` 解析为恰好一个 default bot。
- Happy path：没有 `lark-bots.json` 的 legacy `.env` 作为 enabled `default` bot 启动，不要求 QR 重新注册。
- Happy path：两个 configured bots 解析为两个唯一的 `botKey` entries。
- Happy path：同一个 `appId` 生成同一个 `botKey` candidate。
- Happy path：通过 `bot add` QR 注册会创建一个带 fetched display name 的 generated bot entry。
- Edge case：重复 `botKey` 导致启动失败。
- Edge case：generated/imported bot 不能使用保留的 `default` key。
- Edge case：display-name 变化不改变 `botKey`。
- Edge case：hash collision 会扩展生成后缀，或 fail closed，且不覆盖其他 bot。
...
- Error path：一个 bot WebSocket terminal failure 将 global status 转为 degraded，并标识 bot key。
...
**方法：**
- 给 `ChatThreadBinding` 增加 `botKey`。
- 升级 binding document schema version。
- 新 binding key 为 `botKey + tenantKey + chatId`。
- 现有 schema v1 bindings 迁移到 default bot key。
- 加载 schema v1 bindings 必须是非破坏性的：在内存中归一化为 `botKey = "default"`，并保留 model/personality/style/plan/activeSkill/workspace 字段。
...
- Group unbind 只移除 `botKey + tenantKey + chatId` 下的 `threadId` 和静态执行设置；discovered-group status 保留。
- Group remove 删除 `botKey + tenantKey + chatId` 下的 discovered-group authorization、binding 和 group policy。
- Bot removal 在确认影响范围后，删除所选 `botKey` 拥有的所有本地 records，包括 credentials、owner/admin claim state、discovered groups 和 bindings。

**遵循模式：**
- `BindingStore` 中现有同目录 atomic replacement 和 schema validation。

**测试场景：**
- Happy path：schema v1 binding 作为 default bot binding 加载。
...
- Edge case：重复 `botKey + tenantKey + chatId` 被拒绝。
...
- Error path：unknown keys 或 malformed bot keys fail closed。
...
**方法：**
- 给 `InboundMessage` 增加 `botKey` 和 `chatType`。
- 给 card actions 增加 `botKey`，并校验 action context 与创建该 card 的 bot 匹配。
- 使用 `(botKey, tenantKey, chatId, senderOpenId)` 作为 image aggregation key。
...
**方法：**
- 从 inbound message 中把 `botKey` 存到每个 runtime task。
...
**验证：**
- 每个 Lark side effect 都可以无 payload logging 地追溯到恰好一个 `botKey`。
...
**测试场景：**
- Happy path：doctor 对 legacy config 报告一个 default bot。
- Happy path：包含现有 binding 的 legacy config 在 `default` 下显示同一个 thread binding，且不要求 QR。
- Happy path：`bot migrate-default --write` 探测 `/open-apis/bot/v3/info`，物化 `lark-bots.json` 和升级后的 `bindings.json`，存储 `botOpenId`/`displayName`，并把旧 thread bindings 保留在 `default` 下。
- Happy path：`bot add` 打印一个 QR，获取 bot identity/name，生成一个 internal key，并写入一个 bot，不影响 `default`。
...
| 风险 | 缓解 |
|------|------|
| Cross-bot card action 意外修改另一个 bot 的 task | 在 action normalization 和 token scope 中包含 `botKey`；按 originating card context 校验 |
...
| Binding migration 丢失现有 single-bot installs | Schema v1 迁移到 default bot key，并保持 atomic |
