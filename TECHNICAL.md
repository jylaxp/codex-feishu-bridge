...
网桥通过 `LOG_TO_FILE` 统一控制全部运行日志：
* 当 `LOG_TO_FILE=true`：将 INFO、WARN、ERROR 和脱敏后的飞书 SDK 日志写入轮转日志文件。
* 当 `LOG_TO_FILE=false`：关闭全部运行日志，不写文件，也不输出到标准输出或标准错误。
* CLI 命令结果（例如 `status`、`doctor`）是显式用户输出，不属于运行日志，因此不受该开关影响。
