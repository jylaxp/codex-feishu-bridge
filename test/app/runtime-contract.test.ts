...
test('145 fixture records the expected schema file count', () => {
...
test('one protocol contract records multiple independently verified runtime artifacts', () => {
  ...
  assert.deepEqual(evidence.artifacts[4], {
    capturedAt: '2026-07-24T04:13:20Z',
    codexVersion: '0.146.0-alpha.3',
    binaryName: 'codex',
    binarySha256: '01b89e3cb5b6759c64bc7b47f3f659100e74d743750106ea586b041981f03519',
    distribution: 'ChatGPT.app',
...
test('144 fixture records exact binary provenance', () => {
...
test('recorded 144/145 method comparison matches schemas generated by both binaries', async (t) => {
  ...
  await withTemporaryDirectoryAsync(async (root) => {
    ...
    const codex145Version = parseCodexCliVersion(
      (await execFileAsync(codex145Bin, ['--version'], options)).stdout.trim(),
    ).version;
    if (!['0.145.0-alpha.18', '0.145.0-alpha.27', '0.145.0-alpha.30'].includes(codex145Version)) {
      t.skip('CODEX_145_BIN must select a supported 0.145 binary for this comparison');
      return;
    }
...
test('runtime contract selects built-in smoke-verified aliases through the 145 adapter', () => {
  for (const codexVersion of [
    ...
    const profile = assertCompatibleCodexRuntime(`codex-cli ${codexVersion}`);
...
test('runtime verification reports and cleans up the selected profile', async (t) => {
  ...
  await withTemporaryDirectoryAsync(async (root) => {
    ...
    assert.doesNotMatch(
      readFileSync(join(root, 'config', 'protocol-versions.json'), 'utf8'),
      /schemaDigest/,
    );
...
test('unknown versions remain incompatible until protocol smoke succeeds', async () => {
  await withTemporaryDirectoryAsync(async (root) => {
    ...
    assert.equal(pending.assessment.status, 'incompatible');
    assert.equal(
      pending.config.supportedVersions.some((entry) => entry.codexVersion === '0.146.0-alpha.9.2'),
      false,
    );
    assert.deepEqual(
      pending.detection.compatibility,
      {
        conclusion: '不兼容',
        status: 'incompatible',
...
test('startup auto-supports a smoke-verified unknown version', async () => {
  await withTemporaryDirectoryAsync(async (root) => {
    const codexCwd = join(root, 'workspace');
    const configHome = join(root, 'config');
    const temporaryRoot = join(root, 'runtime');
    ...
    const report = await verifyCodexRuntimeContract(
      ...
      temporaryRoot,
      {
        ...
        protocolSmokeRunner: async (options) => {
          ...
          assert.equal(Object.hasOwn(options.target, 'schemaDigest'), false);
    ...
    assert.doesNotMatch(readFileSync(join(configHome, 'protocol-versions.json'), 'utf8'), /schemaDigest/);
  });
});

test('startup rejects an unknown version when protocol smoke fails', async () => {
  await withTemporaryDirectoryAsync(async (root) => {
    const codexCwd = join(root, 'workspace');
    const configHome = join(root, 'config');
    const temporaryRoot = join(root, 'runtime');
...
test('runtime verification does not invoke schema generation', async () => {
  await withTemporaryDirectoryAsync(async (root) => {
...
test('bundled supported 145 binary reproduces the committed protocol contract', async (t) => {
  ...
  await withTemporaryDirectoryAsync(async (root) => {
    ...
    if (!['0.145.0-alpha.18', '0.145.0-alpha.27', '0.145.0-alpha.30'].includes(codexVersion)) {
      t.skip('CODEX_145_BIN must select a supported 0.145 binary for this fixture replay');
      return;
    }
    assert.equal(
...
throw new Error('generate-json-schema must not be called by runtime compatibility');
`;
  writeFileSync(filePath, source);
  chmodSync(filePath, 0o755);
  return filePath;
