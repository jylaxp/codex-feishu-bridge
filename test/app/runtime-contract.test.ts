...
const schemaDigest145 = '7a5aaea66a649faae713d43313289ddd79b4883086c10875f9031a56ec00bd5c';
const schemaDigest146 = '8535b3371e916d0ea4f2bc62c28a7236323d5f37fd7652184098c90d256c738f';
...
test('recorded 144/145 method comparison matches schemas generated by both binaries', async (t) => {
  ...
  await withTemporaryDirectoryAsync(async (root) => {
    ...
    if (!['0.145.0-alpha.18', '0.145.0-alpha.27', '0.145.0-alpha.30'].includes(codex145Version)) {
      t.skip('CODEX_145_BIN must select a 0.145 schema-compatible binary for this comparison');
      return;
    }
...
test('runtime contract selects built-in smoke-verified aliases through the 145 adapter', () => {
  for (const [codexVersion, schemaDigest] of [
...
test('bundled supported 145 binary reproduces the committed protocol contract', async (t) => {
  ...
  await withTemporaryDirectoryAsync(async (root) => {
    ...
    if (!['0.145.0-alpha.18', '0.145.0-alpha.27', '0.145.0-alpha.30'].includes(codexVersion)) {
      t.skip('CODEX_145_BIN must select a 0.145 schema-compatible binary for this fixture replay');
      return;
    }
    assert.equal(
      builtInProtocolVersionConfig().supportedVersions.some(
