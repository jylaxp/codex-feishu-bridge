...
test('first load seeds built-in versions and later loads read the persisted catalog', () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ['0.144.3', '0.145.0-alpha.18', '0.145.0-alpha.19'],
    );
    assert.equal(reloaded.supportedVersions[2]?.source, 'approved');
  } finally {
